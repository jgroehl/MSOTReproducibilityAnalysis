...
for SCORE, BIGGER_IS_BETTER in zip(SCORES, BIGGER_IS_BETTER_S):
    for day in DAY:
        for paus in PA_OR_US:
            ...
            KEY = "ssim"
            N_BOOTSTRAPS = 100
            TITLE = f"Podium Plot Experiment {day} (using {paus} data)"
            ...
            for pos, idx in zip(position_ranking, range(len(OPERATORS))):
                ...
                plt.scatter(x, y, alpha=1, s=1, c=COLOURS[OPERATORS[pos]-1])
            ...
            plt.savefig(rf"{PATH}\{day}. Runde/{SCORE}_{paus}_podium.png", dpi=300)
            plt.close()
            ...
            plt.ylabel(f"{SCORE} ({'bigger is better' if BIGGER_IS_BETTER else 'smaller is better'})", fontsize=8,
                       fontweight="bold")
            plt.gca().spines[["right", "top"]].set_visible(False)
            plt.savefig(rf"{PATH}\{day}. Runde/{SCORE}_{paus}_values.png", dpi=300)
            plt.close()
